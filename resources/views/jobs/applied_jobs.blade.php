                        ...
                        <div class="jobs_apply_block clearfix">
                            ...
                            <div class="date_block clearfix">
                                <div class="inner_block clearfix">
                                    ...
                                    @if(isset($job->user->first()->pivot->status) && isset($status))
                                        <div class="date_text">
                                            <img src="{{ asset('images/calendar_icon.png') }}" alt="">
                                            <span style="font-family:Serif">Status:

                                                @if(isset($job->user->first()->pivot->status))

                                                    @if($job->user->first()->pivot->status == $status[0]['key'])
                                                        <td>{{$status[0]['value']}}</td>
                                                    @endif
                                                    @if($job->user->first()->pivot->status == $status[1]['key'])
                                                        <td>{{$status[1]['value']}}</td>
                                                    @endif
                                                    @if($job->user->first()->pivot->status == $status[2]['key'])
                                                        <td>{{$status[2]['value']}}</td>
                                                    @endif
                                                    @if($job->user->first()->pivot->status == $status[3]['key'])
                                                        <td>{{$status[3]['value']}}</td>
                                                    @endif
                                                @endif
...
{{--                                        {{$job->user}}--}}
                                        @if(isset($job->user) && isset($logged_user) && count($job->user) > 0)

                                            @foreach ($job->user as $user)
                                                @if(isset($logged_user) && $user->id == $logged_user)
                                                    @if(isset($job->user->first()->pivot))
                                                        @if(isset($job->user->first()->pivot->cv_id))
                                                            <a href="{!! URL::route('download-cv', ['id' => $job->user->first()->pivot->cv_id]) !!}">
                                                                <i class="icon-download-alt"> </i> CV
                                                            </a>
